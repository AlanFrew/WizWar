...
namespace Library {
	static class LibraryFunctions {
		...
		public static List<T> RecursiveCopyList<T>(this List<T> tList) {
			...
			foreach (T t in tList) {
				if (t is ICopiable<T>) {
					...
					MethodInfo inst = t.GetType().GetMethod("MemberwiseClone",
		...
		public static Case FindCase(bool firstCondition, bool secondCondition) {
			if (firstCondition) {
				return secondCondition ? Case.Both : Case.First;
			}

			return secondCondition ? Case.Second : Case.None;
		}
		...
		public static double VectorToAngle(double tX, double tY) {
			if (tX == 0 && tY == 0) {
				...
				return Math.Atan2(tY, tX) + Math.PI;
			}
		}
	}
}
